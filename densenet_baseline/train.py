    ...
    if is_train:
        ...
        return torchvision.transforms.Compose(
            [
                ...
                torchvision.transforms.Normalize(mean=[0.79241839, 0.62130083, 0.79159392], std=[0.15686465, 0.2131858,  0.12528739])
            ]
        )
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./densenet_output_dir',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./densenet_output_dir',
...
def main(args, mode='train', test_image_path=''):
    ...
    if mode == 'train':
        ...
        # 构建模型
        model = timm.create_model('densenet169', pretrained=True, num_classes=5, drop_rate=0.1)
        ...
        model = timm.create_model('densenet169', pretrained=False, num_classes=5, drop_rate=0.1)
        ...
        img_pre_idx = int(class_idx.item())
            ...
            for idx, t_path in tqdm(enumerate(t_patch_path)):
                ...
                for item in json_list:
                    ...
                    for idx, img_path in enumerate(img_paths):
                        img_pre_idx = main(args, mode=mode, test_image_path=img_path)
                        one_anno_all_result.append(img_pre_idx)
